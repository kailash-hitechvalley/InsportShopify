...
trait ShopifyProductMutationTrait
{
    ...
    public function createOrUpdateProductMutation($productData)
    {
        ...
        $variations = $this->createVariantsMutation($productData->variants);
    ...
    private function createVariantsMutation($variantDatas)
    {
        $variants = [];
        $str = [];
        $uniqueOptions = [];
        ...
        if ($totalActivatedLocation == 9) {
            ...
            foreach ($sourceSohs as $sourceSoh) {
                ...
                if (
                    ...
                    || $locationId === "gid://shopify/Location/61496787016" // temp 99 warehouse
                    || $locationId === "gid://shopify/Location/61496819784" // future warehouse
                    || $locationId === "gid://shopify/Location/61496852552" // in transit warehouse
                    // popup store left
                ) {
                    $sohDiff = 0;
                } else {
        ...
        if ($countLocation == 9) {
            foreach ($resArray as $res) {
                ...
                if ($res->node->location->id === $locationId) {
                    ...
                    $sohinShopify = $res->node->available;
